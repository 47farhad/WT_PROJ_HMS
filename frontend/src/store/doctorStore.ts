...
export const useDoctorStore = create<DoctorStore>((set, get) => ({
  ...
  fetchDoctors: async (filters?: DoctorFilters) => {
    try {
      ...
      const response = await axiosInstance.get(`/admin/doctors?${queryString}`);
  ...
  fetchDoctorSchedule: async (id: string) => {
    try {
      set({ loading: true, error: null });
      const response = await axiosInstance.get(`/admin/doctors/${id}/schedule`);
  ...
  updateDoctorSchedule: async (id: string, schedule: Schedule[]) => {
    try {
      set({ loading: true, error: null });
      await axiosInstance.put(`/admin/doctors/${id}/schedule`, { schedule });
