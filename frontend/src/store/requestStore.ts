...
export const useRequestStore = create<RequestStore>((set, get) => ({
  ...
  fetchAllRequests: async (filters?: RequestFilters) => {
    try {
      ...
      const response = await axiosInstance.get(`/admin/requests?${queryString}`);
  ...
  fetchDoctorRequests: async (doctorId: string) => {
    try {
      set({ loading: true, error: null });
      const response = await axiosInstance.get(`/admin/requests/doctor/${doctorId}`);
  ...
  updateRequestStatus: async (requestId: string, status: 'approved' | 'rejected', adminNotes?: string) => {
    try {
      ...
      await axiosInstance.patch(`/admin/requests/${requestId}/status`, {
        status,
        adminNotes
      });
